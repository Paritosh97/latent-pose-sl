...
class VPoser(nn.Module):
    ...
    def encode(self, Pin):
        ...
        logvar = F.softplus(self.bodyprior_enc_logvar(Xout))
        # prevent inf loss
        logvar = torch.max(logvar, torch.ones_like(logvar)*1e-5)
        return torch.distributions.normal.Normal(self.bodyprior_enc_mu(Xout), logvar)
    ...
    def sample_poses(self, num_poses, output_type='aa', seed=None):
        ...
        self.eval()
        pose_embed = np.random.normal(0., 10., size=(num_poses, self.latentD))
        pose_embed[:,0] = 100
        with torch.no_grad():
            Zgen = torch.tensor(pose_embed, dtype=dtype).to(device)
